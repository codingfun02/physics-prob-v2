...
| **질량중심 (COM)** | 질량이 한 점에 모여 있다고 생각했을 때 그 점의 위치. 저울 위에 올렸을 때 **균형이 잡히는 점** |
...
- $\mathbf{r}_{\mathrm{cm}}$: 질량중심 = “각 칸 질량 × 그 칸 위치”를 더한 뒤, 전체 질량으로 나눈 것
...
> **비유:** 케이크를 216조각으로 자르고, 조각마다 질량을 재서 “진짜 질량중심이 어디인지” 구하는 것.
...
중력은 전체적으로 아래로 당기지만, **질량중심이 중심에서 벗어나 있으면** 주사위를 기울이는 **토크**가 생깁니다:
...
질량중심의 높이를 $z_{\mathrm{cm}}$이라 하면, 중력 위치 에너지:
...
- **얼마나 높이에서 떨어뜨리는지** (`DROP_HEIGHT = 6.0` m)
...
| 3 | 높이 6 m, **랜덤 방향**, **랜덤 회전**으로 시작 | `DROP_HEIGHT`, 쿼터니언 |
...
대신:
1. `physics/inertia.py`에서 **총 질량 $M$**, **질량중심**, **관성 모멘트**를 미리 계산
2. **모양은 1개의 큰 정육면체**로 두고, “질량중심이 어디인지”“얼마나 돌리기 어려운지”만 PyBullet에 알려 줌

```python
# bodies.py가 하는 일 (쉬운 설명)
createMultiBody(
    mass = M,                          # 총 질량
    inertialFramePosition = r_cm,      # 질량중심 위치
...
균일 밀도 검증: 질량중심 $\approx (0,0,0)$, `step2_test_inertia.py`로 확인.
...
| `density/grid.py` | 216칸 밀도 저장, 질량·질량중심 계산 |
